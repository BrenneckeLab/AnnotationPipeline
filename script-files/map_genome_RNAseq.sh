...
STAR --runThreadN $redCORES \
    ...
    --outSAMmode NoQS --readFilesCommand cat  --alignEndsType Local --twopassMode Basic   \
    --outReadsUnmapped Fastx  --outMultimapperOrder Random --outSAMtype SAM --outFilterMultimapNmax 1000 --winAnchorMultimapNmax 2000 --limitOutSAMoneReadBytes 90000000 \
...
if [[ $SLAM == Y ]]; then
  ...
  for MAPtype in uniq multi; do
      ...
      ' ${locTMP}${MAPtype}.sam | sort -k1,1 >> ${libFOLDER}SNPsummary.txt 
  done
...
if [[ $exportBAM == Y ]]; then 
  samtools view -bS ${locTMP}uniq.sam > ${libFOLDER}${NAME}.uniq.collapsed.bam
  samtools view -bS ${locTMP}multi.sam > ${libFOLDER}${NAME}.multi.collapsed.bam
fi
